...
// tier 1 core / 2 nice / 3 filler (docs/journey-tiers.json). Tier-3 cards are hidden from the
// live library (still resolvable by id so a moment saved before the hide keeps working).
const tiers = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'docs', 'journey-tiers.json'), 'utf8'));
const rt = cards.map(c => ({ id: c.id, stage: c.stage, section: c.section, sub: c.subsection, cap: c.caption, timing: c.timing, who: c.who_code, slug: c.slug, art: c.slug + '.webp', t: tiers[c.id] || 1 }));
